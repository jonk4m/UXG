...
bool YATG::append_rows_to_uxg_file(QHash<QString,int> indexes, double timeScalingFactor){
    ...
    double pdwSetupTime = 0.00000027; //270ns
    double currentTOA = 0;
    QString outputData;
    ...
    while(true){
        ...
        for(int i = 0; i < count; i++){
            ...
            //if it's the first PDW add the setup time so the next pdw accounts for it in it's TOA
            if(operationVal == "1"){
                currentTOA += pdwSetupTime * (timeScalingFactor);
            }
            double valueOfPri = rowList.at(indexes.value("Pri")).toDouble();
            double setupTimePerPDW = pdwPadding * (timeScalingFactor); //adds in 270ns for the setup time
            currentTOA += valueOfPri + setupTimePerPDW;
